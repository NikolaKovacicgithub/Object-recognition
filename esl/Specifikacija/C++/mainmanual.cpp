...
void processColorContours(const cv::Mat& img, const cv::Scalar& lower, const cv::Scalar& upper, const cv::Scalar& color, const std::string& label, int& objects_count, cv::Mat& marked_image) {
    ...
    for (int y = 0; y < img.rows; ++y) {
        for (int x = 0; x < img.cols; ++x) {
            ...
            if (pixel[0] >= lower[0] && pixel[1] >= lower[1]
                && pixel[2] >= lower[2] && pixel[0] <= upper[0] 
                && pixel[1] <= upper[1] && pixel[2] <= upper[2]) {
            ...
            if (mask.at<uchar>(y, x) == 255) {
                ...
                for (int i = -5; i <= 5; ++i) {
                    for (int j = -5; j <= 5; ++j) {
                        int newY = y + i;
                        int newX = x + j;
                        if (newY >= 0 && newY < closed_mask.rows &&
                            newX >= 0 && newX < closed_mask.cols) {
            ...
            if (closed_mask.at<uchar>(y, x) == 255) {
                ...
                for (int i = -5; i <= 5; ++i) {
                    for (int j = -5; j <= 5; ++j) {
                        int newY = y + i;
                        int newX = x + j;
                        if (newY >= 0 && newY < closed_mask.rows && newX >= 0
                            && newX < closed_mask.cols) {
